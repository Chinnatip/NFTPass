...
export default ({ src, nfts_lists, creators }: {
  src: NFT,
  nfts_lists: NFT[],
  creators: Creator[]
}) => {
  ...
  return <>
    ...
    <div className="bottom-0 left-0 bg px-2 pt-2 md:text-s text-sm font-semibold" style={{borderRadius: '0px 0px 8px 8px'}}>
      <img
        className="h-6 w-6 rounded-full border-1 border-white inline"
        src={src?.creator_image} alt=""/>
      <span className="ml-2">{src?.creator_name}</span></div>
