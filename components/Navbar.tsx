...
const Navbar = ({ current , show=true }: {current: number, show?: boolean}) => {
  ...
  return <div className="flex items-center mb-10">
    ...
    { show && <div className="absolute p-1 bg-white rounded-full shadow-nft text-center float-central md:flex hidden" >
      ...
      <button onClick={() => Router.push('/claim')} className={`focus:outline-none py-2 text-sm rounded-full px-5 ${current == 1 ?  'bg-black text-white': 'text-black' }`}>Your Page</button>
    </div>}
